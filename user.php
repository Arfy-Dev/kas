...
<script>
    // Mendapatkan semua elemen dengan nama "nama_lengkap"
    var Inputs = document.getElementsByName("nama_lengkap");

    // Menangani setiap elemen dengan nama "nama_lengkap"
    for (var i = 0; i < namaInputs.length; i++) {
        var input = namaInputs[i];

        // Menambahkan event listener untuk memeriksa input saat nilai berubah
        input.addEventListener("input", function () {
            var inputValue = this.value;
            var regex = /^[A-Za-z .]+$/; // Hanya huruf, spasi, dan titik diperbolehkan

            if (!regex.test(inputValue)) {
                alert("Input hanya boleh mengandung huruf, spasi, dan titik.");
                this.value = ''; // Mengosongkan input jika tidak valid
            }
        });
    }
</script>
